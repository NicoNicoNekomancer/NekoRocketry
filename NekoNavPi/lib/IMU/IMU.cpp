...
to read data you need to send a dummy byte before you actually get the data.
reading is first bit shifted, so | 0x80
multi read/writes are supported, based on a n+1 system starting at the initial address.
  -reads are jank as they shift the index x 1 in this code as i have the dummy built in
...
uint16_t IMU::readRegPage(uint8_t regAddress, uint8_t page){
  ...
  SPI.transfer(dummy);
  //collect responce
  uint8_t result1 = SPI.transfer(dummy);
  //collect the other half
  uint8_t result2 = SPI.transfer(dummy);
  // Deselect the device by bringing CSB high
  digitalWrite(_csPin, HIGH);
  // End SPI transaction
  SPI.endTransaction();
  ...
  //combine to 16 bit
  uint16_t result = ((uint16_t)result2 << 8) | result1;
  return result;
}
void IMU::writeRegPage(uint8_t regAddress, uint8_t page, uint8_t data1, uint8_t data2){
  //Write to the page
  // Begin SPI transaction
  SPI.beginTransaction(_spiSettings);
  // Select the device by bringing CS low
...
void IMU::initIMU() {
  ...
  //set to odr 1.6k, norm, perf, perf
  writeReg(GYR_CONF, 0xEC);
  delayMicroseconds(450);
  //set to odr 1.6k, CIC_avg8, perf
  writeReg(ACC_CONF, 0xBC);
  delayMicroseconds(450);
  //set to +-8g
  writeReg(ACC_RANGE, 0x02);
  delayMicroseconds(450);
  //2000 dps, no ois since it dosent matter
  writeReg(GYR_RANGE, 0x00);
  delayMicroseconds(450);
  writeRegPage(0x34, 1, 0x88, 0x02);
  delayMicroseconds(450);
}
...
// Begin SPI transaction
  ...
  IMUTemp =(float)(((float)((int16_t)(twosComplement(TempRaw1, TempRaw2)))) * 0.001953125) + 23.0;
  ...
  //read CAS. this is on page 0 of features. Technically its set to 0 already but whatever
  GYR_CAS = readRegPage(0x3C, 0x00);
  ...
  GyroXRAW = GyroXRAW - (GYR_CAS_Twos * GyroZRAW * 0.001953125);
