...
Madgwick::Madgwick() {
	beta = betaDef;
  //initial starting values. they will be changed in the begin
	q0 = 1.0f;
	q1 = 0.0f;
	q2 = 0.0f;
	q3 = 0.0f;
...
void Madgwick::update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float &Q1, float &Q2, float &Q3, float &Q4) {
  unsigned long currentMicros = micros();  // get the current time
  float timeDifference = (currentMicros - previousMicros) * 0.000001;
  previousMicros = currentMicros;
	...
	q0 += qDot1 * timeDifference;
	q1 += qDot2 * timeDifference;
	q2 += qDot3 * timeDifference;
	q3 += qDot4 * timeDifference;
  ...
  //I'm sorry this was done this way. Too lazy to change the other variables despite the fact that I can just replace all
  Q1 = q0;
  Q2 = q1;
  Q3 = q2;
  Q4 = q3;
  Serial.println(timeDifference, 6);
}
